...
for agent, target in zip(range(number_of_agents), target_positions):
    ...
    agents.append(Agent(arena, agents, width, height, agent_name, target[0], target[1], inner, outer, max_speed, max_length,
                        starting_angle, obstacle_radius, text_font, agent_text_size, white, radius, epsilon, learning_rate, 
...
for episode in range(episodes):
    ...
    while run_episode:
        ...
        for agent in agents:
            ...
            agent.update_acceleration(agent, agents, obstacle_position, wander_magnitude, avoid_magnitude, target_magnitude)
